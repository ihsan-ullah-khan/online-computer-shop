...
@section('content')
    <article class="max-w-6xl lg:max-w-8xl mx-auto mt-8 lg:grid space-y-6 lg:mt-20 lg:grid-cols-12 gap-x-10">
        <div class="col-span-2 lg:text-center lg:pt-14 mb-10">
            <a class="text-lg font-semibold hover:text-white hover:bg-blue-500 px-4 py-1 mt-4 rounded-lg" href="{{ route('orders.index') }}">My Orders</a>
            <a class="text-lg font-semibold hover:text-white hover:bg-blue-500 px-4 py-1 mt-4 rounded-lg" href="{{ route('orders.delivered') }}">Delivered</a>
            <a class="text-lg font-semibold hover:text-white hover:bg-blue-500 px-4 py-1 mt-4 rounded-lg" href="{{ route('orders.returned') }}">Returned</a>
        </div>
        ...
        <div class="col-span-10 w-full">
            <div class="w-full overflow-x-auto">
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                    <tr
                        class="text-xs font-semibold tracking-wide w-full text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800"
                    >
                            ...
                            <td class="px-4 py-3 w-12">
                                <div class="flex justify-end space-x-4 text-sm">
                                    <form method="POST" action="{{ route('orders.update', $order) }}">
                                        ...
                                        <x-button>Recieved</x-button>
                                    </form>
                                    <form method="POST" action="{{ route('order.returned', $order->id) }}">
                                        @method('patch')
                                        @csrf
                                        <input type="hidden" name="status" value="R">
                                        <x-button>Return</x-button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                        ...
                        <tr class="text-gray-700 dark:text-gray-400 bg-orange-100">
                            <td colspan="9" class="px-4 py-3">
                                <div class="flex justify-center text-sm">
                                    <p class="font-semibold">No Orders Available.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
